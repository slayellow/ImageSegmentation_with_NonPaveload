...
class ModelDeepLab_v3:

    def __init__(self, block_list=[3,4,23,3]):
    ...
    def build_model(self, image, classes, first=256):
        ...
        conv1 = self.set_conv1_block(bgr, first)

        # conv2 -> maxpool 3x3, stride 2 --> Result : 1/8 scale
        #
        ...
        conv2 = self.set_residual_block(maxpool, first, self.block_list[0], is_half=True, name="conv2")

        # conv3 --> Result : 1/16 scale
        #
        # [ kernel 1x1, channels 128    ]   stride : 2
        # [ kernel 3x3, channels 128    ]  x 4
        # [ kernel 1x1, channels 512    ]
        conv3 = self.set_residual_block(conv2, first*2, self.block_list[1], is_half=True, name="conv3")

        # conv4 --> Result : 1/16 scale
        # Add Atrous Convolution
        # [ kernel 1x1, channels 256    ]   stride : 1
        # [ kernel 3x3, channels 256    ]  x 23
        # [ kernel 1x1, channels 1024    ]
        conv4 = self.set_residual_block(conv3, first*4, self.block_list[2], name="conv4")

        # conv5 --> Result : 1/16 scale
        # Add Atrous Convolution
        # [ kernel 1x1, channels 512    ]   stride : 1
        # [ kernel 3x3, channels 512    ]  x 3
        # [ kernel 1x1, channels 2048    ]
        conv5 = self.set_atrous_residual_block(conv4, first*8, self.block_list[3], [2,4,8], name="conv5")
    ...
    def set_conv1_block(self, data, channel):
        W1 = tensorflow_util.get_init_weight([3, 3, data.shape[3], (int)(channel/4)], name="conv1_W1")
        b1 = tensorflow_util.get_init_bias([(int)(channel/4)], name="conv1_b1")
    ...
    def set_atrous_residual_block(self, data, channel, block_num, atrous, name="residual"):
        data_channel = data.shape[3]
        layer_data = data
        for i in range(block_num):
            if i==0:
            ...
            W2 = tensorflow_util.get_init_weight([3, 3, relu1.shape[3], (int)(channel / 4)],
                                                 name=name + "_residual_" + str(i) + "W2")
            b2 = tensorflow_util.get_init_bias([channel / 4], name=name + "_residual_" + str(i) + "b2")
            conv2 = tensorflow_util.set_atros_conv_layer(relu1, b2, W2, rate=atrous[i])
            bn2 = tensorflow_util.set_batch_normalization(conv2, name=name + "_residual_" + str(i) + "bn2")
            relu2 = tensorflow_util.set_relu(bn2, name=name + "_residual_" + str(i) + "relu2")
